...
export default function HomePage() {
  return <main className="public-site">
    ...
    <section className="public-section domains-section" id="domaines"><div className="public-container domains-layout"><div className="section-heading left"><span>DOMAINES D’INTERVENTION</span><h2>Une première assistance pour les situations du quotidien.</h2><p>Particulier ou professionnel, sélectionnez le motif correspondant à votre situation.</p><Link href="/impayes" className="text-link">Découvrir LEXIA Impayés →</Link></div><div className="domain-grid">{domains.map(([icon,title,description])=><article key={title}><div>{icon}</div><h3>{title}</h3><p>{description}</p></article>)}</div></div></section>
    <section className="public-section pricing-section" id="tarifs"><div className="public-container"><div className="section-heading centered light-heading"><span>TARIFICATION CLAIRE</span><h2>Un coût connu dès le départ.</h2><p>Toute prestation complémentaire doit être présentée et acceptée avant paiement.</p></div><div className="pricing-grid"><article className="price-card"><div className="price-card-top"><span>PARTICULIER</span><div><strong>13 €</strong><small>TTC / dossier</small></div></div><p>Pour les litiges et démarches de la vie quotidienne.</p><Link href="/inscription" className="public-button public-button-gold">Commencer</Link></article><article className="price-card featured-price"><div className="popular-tag">PROFESSIONNELS</div><div className="price-card-top"><span>ENTREPRISE</span><div><strong>29 €</strong><small>TTC / dossier</small></div></div><p>Pour les sociétés, indépendants et associations.</p><Link href="/impayes" className="public-button public-button-light">Service Impayés</Link></article></div></div></section>
    <section className="public-section final-cta"><div className="public-container final-card"><div><span>VOTRE ESPACE JURIDIQUE</span><h2>Une situation vous préoccupe ? Commencez par nous l’expliquer.</h2><p>Créez votre compte, transmettez les informations utiles et suivez votre dossier en ligne.</p></div><div className="final-actions"><Link href="/inscription" className="public-button public-button-gold">Créer mon dossier</Link><Link href="/impayes" className="final-login">Déclarer un impayé →</Link></div></div></section>
    <footer className="public-footer"><div className="public-container footer-main"><div><Link href="/" className="footer-logo">LEXIA<span>.</span></Link><p>L’assistance juridique en ligne, simple, humaine et sécurisée.</p></div><div><b>Services</b><Link href="/impayes">LEXIA Impayés</Link><Link href="/conseils-juridiques">Conseils juridiques</Link></div><div><b>Mon espace</b><Link href="/inscription">Créer un compte</Link><Link href="/connexion">Se connecter</Link><Link href="/nouveau-dossier">Déposer un dossier</Link></div><div><b>Informations</b><Link href="/confidentialite">Confidentialité</Link><Link href="/conditions">Conditions générales</Link><Link href="/mentions-legales">Mentions légales</Link></div></div><div className="public-container footer-bottom"><span>© 2026 LEXIA. Tous droits réservés.</span></div></footer>
  </main>;
}
