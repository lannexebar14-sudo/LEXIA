    ...
    <section className="admin-email-composer">
      ...
      <div className="admin-email-compose-grid">
        <div className="admin-email-form-card">
          ...
          <label>
            <span>Message</span>
            <textarea value={message} onChange={(event) => setMessage(event.target.value)} maxLength={10000} rows={9} placeholder="Rédigez le message destiné au client…" />
          </label>
          ...
          <p className="admin-email-security">La clé Resend reste chiffrée côté serveur. Aucun client ne peut accéder à la fonction d’envoi.</p>
        </div>

        <div className="admin-email-live-preview">
          <header>
      ...
      <section className="admin-email-history">
        <header><div><small>HISTORIQUE</small><h3>Derniers e-mails envoyés</h3></div><span>{history.length}</span></header>
        <div>
